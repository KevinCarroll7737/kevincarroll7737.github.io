...
Meh, it doesn't seem that hard... Let's provide this program with everything it needs for the first two user inputs. Then, simply overflow the third one, to overwrite the user variable with `root`. To do this, it is necessary to determine the memory address of the `user` variable.
